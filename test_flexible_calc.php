...
echo "=== Test new flexible grading query ===\n";
$query = "
SELECT ce.student_id,
    ROUND(
        (
            COALESCE(AVG(CASE WHEN gcc.component_type='midterm' THEN (COALESCE(sfg.raw_score,0)/gcc.max_score*100) ELSE NULL END), 0)
            * (40.0 / 100.0)
        ) +
        (
            COALESCE(AVG(CASE WHEN gcc.component_type='finals' THEN (COALESCE(sfg.raw_score,0)/gcc.max_score*100) ELSE NULL END), 0)
            * (60.0 / 100.0)
        )
