...
echo "=== Testing FINAL FIXED COA Query (with class_code filter in JOIN) ===\n";
$class_code = '25_T2_CTAPROJ1_INF223';
$course_code = 'CTAPROJ1';
...
$query = "SELECT 
           ...
           INNER JOIN grading_components gc ON gc.class_code = ? AND gc.course_code = co.course_code
...
$stmt->bind_param("sss", $class_code, $class_code, $course_code);
if (!$stmt->execute()) {
...
if ($count == 8) {
    echo "✓ SUCCESS: Correct number of rows (only CO1 assessments for this class)\n";
} else {
    echo "✗ PROBLEM: Expected 8 rows (5 Quiz + 3 Classwork), got $count\n";
}
