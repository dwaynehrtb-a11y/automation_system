...
echo "=== Full COA query results (raw data) ===\n";
$query = "SELECT 
    co.co_number,
    co.co_description,
    gc.component_name,
...
while($row = $result->fetch_assoc()) {
    echo "CO{$row['co_number']}: {$row['component_name']} (target: {$row['performance_target']}%) - Met: {$row['students_met_target']}/{$row['total_students']}\n";
}
$stmt->close();
